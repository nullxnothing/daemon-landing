...
export function SolanaDevelopmentDoc() {
  return (
    <>
      ...
      <DocSubheading>
        Wallets, RPC, MCPs, token launches, transaction debugging, readiness checks, and protocol integrations live in one
        Solana workspace.
      </DocSubheading>

      <H2 id="tool-map">Solana Tool Map</H2>
      <Paragraph>
        DAEMON exposes Solana as a set of focused tools instead of one oversized panel. Use this map when you need to find
        the right surface for a project task.
      </Paragraph>
      <Table
        headers={["Tool", "Where", "What it covers"]}
        rows={[
      ...
      <H2 id="solana-workspace">Solana Workspace</H2>
      <Paragraph>
        The Solana Workspace groups the most common project decisions into five views. It is the place to check whether
        DAEMON sees the project, which transport is active, and which execution paths are ready.
      </Paragraph>
      <Table
        headers={["View", "Use it for"]}
        rows={[
      ...
      <Paragraph>
        Project Readiness turns the active repository into a concrete checklist. It is useful when a Solana app is present
        but the wallet, RPC, MCP, or first agent action is not fully wired.
      </Paragraph>
      <CardGrid>
      ...
      <Paragraph>
        The wallet route is shared by launch, swap, replay, dashboard, and agent actions. DAEMON tracks whether the project
        has a usable signer, assigned wallet, and provider path before it sends transactions.
      </Paragraph>
      <List
        items={[
      ...
      <H2 id="mcp-agent-tools">MCPs And Agent Skills</H2>
      <Paragraph>
        The Solana Workspace also tracks MCP servers and installed skills so agents can read chain data, scaffold code, and
        act through the same wallet and RPC assumptions as the UI.
      </Paragraph>
      <Table
      ...
      <H2 id="jupiter-swaps">Transactions And Swaps</H2>
      <Paragraph>
        Jupiter swaps, wallet sends, recovery flows, and launch transactions run through the shared execution pipeline.
        DAEMON keeps these paths tied to the active wallet, RPC provider, and session history.
      </Paragraph>
      <Hint type="warning">
        Before mainnet writes, verify signer state, destination accounts, ATA creation, slippage, blockhash freshness,
        priority fees, compute limits, and transaction size.
      </Hint>

      <H2 id="launch-tokens">Token Launch And Protocols</H2>
      <Paragraph>
        Token Launch uses the same runtime as the rest of the Solana tools. Pump.fun, Raydium, and Meteora are native launch
        surfaces, while the integrations catalog guides setup for broader protocol work.
      </Paragraph>
      <CardGrid>
      ...
      <H2 id="debugging-testing">Debugging And Testing</H2>
      <Paragraph>
        The Debug view keeps local Solana tooling visible so project setup issues are caught before an agent or launch flow
      ...
      <H2 id="scanner-replay">Block Scanner And Replay</H2>
      <Paragraph>
        Block Scanner and Replay are the investigation tools. Use them to inspect accounts, review signatures, replay
        transactions with context, and hand the result to an agent when debugging needs code changes.
      </Paragraph>
      ...
      <H2 id="session-registry">Activity And Session Registry</H2>
      <CardGrid>
        <InfoCard title="What&apos;s Recorded">Launch attempts, token imports, swap signatures, replay notes, agent actions, and dashboard additions.</InfoCard>
        <InfoCard title="Why It Matters">The registry lets you return to project context without rebuilding state from memory.</InfoCard>
      </CardGrid>
    </>
  );
}
